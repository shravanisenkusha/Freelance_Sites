...
const Testimonials = () => {
  ...
  const TestimonialCard = ({ testimonial }) => (
    <div className="bg-[#fcfbf5] rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 overflow-hidden flex flex-col md:flex-row p-6 gap-6">
      ...
      <div className="flex-1 flex flex-col justify-center">
        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-3 mb-4 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-purple-800 text-sm md:text-base">Worked on {testimonial.project}</span>
          </div>
        </div>
        <div className="mb-4">
  ...
  const MentorImageCard = ({ mentor, idx }) => {
    ...
    const sizeClass = idx === 4 ? 'md:w-52 md:h-52 w-32 h-32' : 'md:w-60 md:h-60 w-36 h-36';
    const extraTransform = idx === 3 ? 'translate-y-[-16px]' : '';

    return (
      <div className={`relative group flex items-center justify-center -m-14 md:-m-20 ${extraTransform}`}>
        {showBelow ? (
